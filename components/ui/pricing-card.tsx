...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      className={cn(
        "relative w-full max-w-xs rounded-lg bg-card dark:bg-transparent",
        "p-1.5 shadow-md shadow-muted/50 backdrop-blur-xl",
...
function Header({
  ...
  return (
    <div
      className={cn(
        "relative mb-4 rounded-md border bg-muted/80 p-4 dark:bg-muted/50",
        className
      )}
      {...props}
    >
      ...
      {glassEffect && (
        <div
          ...
          style={{
            background:
              "linear-gradient(180deg, rgba(255,255,255,0.04) 0%, rgba(255,255,255,0.03) 40%, rgba(0,0,0,0) 100%)",
          }}
        />
      )}
      {children}
