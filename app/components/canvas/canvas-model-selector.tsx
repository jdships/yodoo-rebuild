...
export function CanvasModelSelector({
  ...
  const handleModelToggle = (modelId: string, isLocked: boolean) => {
    ...
    const minModels = 1; // Canvas allows minimum 1 model (unlike multi-chat which requires 2)
  ...
  const renderModelItem = (model: ModelConfig) => {
    ...
    const minModels = 1;
    const isAtMinimum = selectedModelIds.length <= minModels;
  ...
  const trigger = (
    <Button
      className={cn(
        "min-w-[80px] sm:min-w-[200px] justify-between rounded-full dark:bg-secondary px-2 sm:px-3",
        className
      )}
            ...
            >
              {(() => {
                ...
                return provider?.icon ? (
                  ...
                  >
                    <provider.icon className="size-4 sm:size-5 flex-shrink-0" />
                  </motion.div>
                ) : null;
              })()}
              <motion.span
      ...
      <Tooltip>
        ...
        >
          ...
          >
            ...
            <div className="flex h-full flex-col space-y-0 overflow-y-auto px-1 pt-0 pb-0 gap-px">
              {isLoadingModels ? (
                <div className="flex h-full flex-col items-center justify-center p-6 text-center">
                  <p className="mb-2 text-muted-foreground text-sm">
                    Loading models...
