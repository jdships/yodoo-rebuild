...
export const Suggestions = memo(function Suggestions({
  ...
  // Use prop activeCategory if provided (from slash commands), otherwise use internal state
  const activeCategory = propActiveCategory ?? internalActiveCategory;

  if (!value && activeCategory !== null && !propActiveCategory) {
  ...
  const handleCategoryClick = useCallback(
    (suggestion: { label: string; prompt: string }) => {
      setInternalActiveCategory(suggestion.label);
  ...
  return (
    <AnimatePresence mode="wait">
      {showCategorySuggestions ? suggestionsList : (activeCategory === null ? suggestionsGrid : null)}
    </AnimatePresence>
  );
});
