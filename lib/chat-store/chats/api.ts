...
export async function createNewChat(
  ...
  try {
    ...
    if (!res.ok) {
      ...
      if (res.status === 429) {
        console.log("Detected 429 status, redirecting to upgrade page");
        // Redirect immediately instead of throwing an error
        const errorMessage = encodeURIComponent(responseData.error || "Usage limit reached");
        
        // Use setTimeout to allow current execution to complete before redirect
        setTimeout(() => {
          window.location.href = `/upgrade?error=${errorMessage}`;
        }, 0);
        
        // Return a dummy chat to prevent further errors
        return {
          id: "temp-" + Date.now(),
