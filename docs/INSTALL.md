# Yodoo Installation Guide

Yodoo is an AI chat app with multi-model support.
...
Yodoo supports BYOK functionality, allowing users to securely store and use their own API keys for AI providers. To enable this feature, you need to configure an encryption key for secure storage of user API keys.
...
Ollama allows you to run AI models locally on your machine. Yodoo has built-in support for Ollama with automatic model detection.
...
### Yodoo + Ollama Integration

Yodoo automatically detects all models available in your Ollama installation. No additional configuration is needed!

**Features:**

- **Automatic Model Detection**: Yodoo scans your Ollama instance and makes all models available
...
By default, Yodoo connects to Ollama at `http://localhost:11434`. This works for local installations.
...
Yodoo includes a settings interface where you can:
...
For a complete Docker setup with both Yodoo and Ollama:
...
docker run -d -v ollama:/root/.ollama -p 11434:11434 --name ollama ollama/ollama
docker run -p 3000:3000 -e OLLAMA_BASE_URL=http://ollama:11434 Yodoo
```
...
1. Refresh the models list in Yodoo settings
2. Check Ollama has models: `ollama list`
3. Restart Yodoo if models were added after startup
...
git clone https://github.com/jdships/yodoo-ai.git
cd Yodoo
...
For a complete setup with both Yodoo and Ollama running locally, use the provided `docker-compose.ollama.yml`:

```bash
# Start both Yodoo and Ollama services
...
- **Proper networking** between Yodoo and Ollama
- **Volume persistence** for Ollama models

The Ollama service will be available at `http://localhost:11434` and Yodoo will automatically detect all available models.
...
The easiest way to deploy Yodoo is using Vercel:
...
You can customize various aspects of Yodoo by modifying the configuration files:
